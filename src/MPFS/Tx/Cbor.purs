...
readArgument r 27 = do
  -- 8-byte: read as two 32-bit halves
  hi <- readArgument r 26
  lo <- readArgument r 26
  pure $ mul32 hi + lo
readArgument _ 31 = pure (-1) -- indefinite length
readArgument _ info =
  pure 0 -- should not happen in well-formed CBOR
...
skip r = do
  ...
  case major of
    ...
    6 -> skip r -- tag: skip inner
    7 -> pure unit -- simple/float
    _ -> pure unit
...
-- | Decode Plutus Data from CBOR bytes at the current
-- reader position.
...
readInputs r = do
  b <- peekByte r
  let major = b `shr` 5
  -- May have tag 258 (set semantics)
...
readDatum r = do
  ...
  if major == 4 then do
    ...
    if tag == 0 then do
      ...
      -- Inline datum: tag(24, cborBytes)
      _ <- readTag r -- 24
      datumBytes <- readBytes r
      -- Decode the inner CBOR as PlutusData
...
readRedeemers r = do
  ...
  go arr n = do
    ...
    -- Value: [data, exUnits]
    _ <- readArrayLen r
    -- Capture raw CBOR and decode PlutusData
...
-- | Decode Plutus Data from raw CBOR bytes.
...
-- | Decode a CBOR-encoded Cardano transaction.
