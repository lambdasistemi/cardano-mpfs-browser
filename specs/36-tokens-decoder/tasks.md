...
- [ ] T036-S1 Capture the bumped devnet-server `GET /tokens` response shape used
  by the same-revision e2e server.
- [ ] T036-S1 Add a RED assertion in `test/Test/MPFS/ClientSpec.purs` that
  exercises that response shape and fails on the old top-level-array decoder.
- [ ] T036-S1 Adapt `src/MPFS/Client.purs` and/or
  `src/MPFS/Client/Types.purs` so `Client.getTokens` decodes the new response
  and still returns `Array TokenId`.
- [ ] T036-S1 Run the focused test command and `./gate.sh`; commit one
